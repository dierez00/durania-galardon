...
export async function GET(
  ...
  const docResult = await supabaseAdmin
    .from("upp_documents")
    .select("file_storage_key, tenant_id, upp_id")
    .eq("id", id)
    .maybeSingle();

  if (docResult.error) {
    return apiError("UPP_DOCUMENT_LOOKUP_FAILED", docResult.error.message, 500);
  }
  if (!docResult.data || docResult.data.tenant_id !== auth.context.user.tenantId) {
    return apiError("UPP_DOCUMENT_NOT_FOUND", "Documento no encontrado", 404);
  }
  ...
  if (signedError || !signed?.signedUrl) {
    return apiError(
      "SIGNED_URL_FAILED",
      signedError?.message ?? "No se pudo generar URL firmada",
      500
    );
  }
