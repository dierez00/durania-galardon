...
export async function GET(
  ...
  try {
    const vaccinations = await repository.listVaccinations(id);
    return apiSuccess({ vaccinations });
  } catch (error) {
    return apiError(
      "VACCINATIONS_QUERY_FAILED",
      error instanceof Error ? error.message : "No fue posible cargar vacunaciones.",
      500
    );
  }
}
