...
export default function MvzProfilePage() {
  const [loading, setLoading] = useState(true);
  const [accountSaving, setAccountSaving] = useState(false);
  const [profileSaving, setProfileSaving] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");
  const [payload, setPayload] = useState<MvzProfilePayload["data"]>();
  ...
  const load = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/mvz/profile", {
        headers: {
      ...
      const body = (await response.json()) as MvzProfilePayload;
  ...
  const assignedProjects = payload?.scope?.assignedProjects ?? [];
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {loading ? "-" : payload?.scope?.totalAssignedProjects ?? assignedProjects.length}
            </p>
          </CardContent>
        </Card>
        <Card>
      ...
      {payload?.domainProfile ? (
        <Card>
          <CardHeader>
            <CardTitle>Ficha profesional MVZ</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="licenseNumber">Cedula / licencia</Label>
              <Input id="licenseNumber" value={payload.domainProfile.licenseNumber ?? ""} readOnly />
            </div>
            <div className="space-y-2">
              ...
              <TableBody>
                {assignedProjects.map((project) => (
                  <TableRow key={project.id}>
                    ...
                    <TableCell>{project.producerName ?? "-"}</TableCell>
                    <TableCell>{project.ranchStatus}</TableCell>
                    <TableCell>
                      {project.assignedAt
