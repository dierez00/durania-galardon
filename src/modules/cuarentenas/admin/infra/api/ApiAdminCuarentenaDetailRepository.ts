import type { IAdminCuarentenaDetailRepository } from "@/modules/cuarentenas/admin/domain/repositories/IAdminCuarentenaDetailRepository";
  ...
  implements IAdminCuarentenaDetailRepository
{
  ...
  async updateInfo(
    ...
    if (!resp.ok) {
      const json = (await resp.json().catch(() => ({}))) as { message?: string };
      throw new Error(json.message ?? "Error al guardar informaciÃ³n.");
    }
  }
}
