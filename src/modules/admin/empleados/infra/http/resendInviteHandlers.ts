...
export async function POST(request: Request) {
  const auth = await requireAuthorized(request, {
    ...
    resource: "admin.employees.resend_invite",
  });
  if (!auth.ok) {
  ...
  if (profileResult.error || !profileResult.data?.email) {
    return apiError("ADMIN_EMPLOYEE_PROFILE_NOT_FOUND", "No fue posible resolver el correo del empleado.", 404);
  }
  ...
  if (
    ...
    return apiError(
      "ADMIN_EMPLOYEE_INVITE_RESEND_NOT_ALLOWED",
  ...
  try {
    ...
    return apiError(
      "ADMIN_EMPLOYEE_INVITE_RESEND_FAILED",
