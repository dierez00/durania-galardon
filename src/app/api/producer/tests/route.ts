...
export async function GET(request: Request) {
  ...
  const [animalsResult, typesResult] = await Promise.all([
    animalIds.length > 0
      ? supabaseAdmin.from("animals").select("id,siniiga_tag,status").eq("tenant_id", auth.context.user.tenantId).in("id", animalIds)
      : Promise.resolve({ data: [], error: null }),
    testTypeIds.length > 0
      ? supabaseAdmin.from("test_types").select("id,key,name").in("id", testTypeIds)
      : Promise.resolve({ data: [], error: null }),
  ]);
