...
async function listMemberships(tenantId: string) {
...
export async function GET(request: Request) {
  ...
  try {
    const memberships = await listMemberships(auth.context.user.tenantId);
...
export async function POST(request: Request) {
  ...
  if (membershipLookup.data) {
    ...
    const updateMembership = await supabaseAdmin
      .from("tenant_memberships")
      .update({ status })
      .eq("id", membershipId);
    ...
    const createMembership = await supabaseAdmin
      .from("tenant_memberships")
      .insert({
