...
export async function GET(request: Request) {
  const auth = await requireAuthorized(request, {
    ...
    permissions: ["mvz.notifications.read"],
    resource: "mvz.notifications",
  });
  if (!auth.ok) {
  ...
  const notificationsResult = await supabase
    ...
    .or(`user_id.eq.${auth.context.user.id},role_key.eq.${auth.context.user.role}`)
    .order("created_at", { ascending: false })
    .limit(200);

  if (notificationsResult.error) {
    return apiError("MVZ_NOTIFICATIONS_QUERY_FAILED", notificationsResult.error.message, 500);
  }
