...
function BoolChip({ value, label }: { value: boolean | null; label: string }) {
  if (value === null) return <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><MinusCircle className="h-3 w-3" />{label}</span>;
  if (value) return <span className="inline-flex items-center gap-1 text-xs text-green-600"><CheckCircle2 className="h-3 w-3" />{label}</span>;
  return <span className="inline-flex items-center gap-1 text-xs text-red-500"><XCircle className="h-3 w-3" />{label}</span>;
}
...
export default function ProducerExportacionesPage() {
  ...
  const uppName = (uppId: string | null) =>
    !uppId ? "-" : upps.find((u) => u.id === uppId)?.name ?? uppId.slice(0, 8);
                ...
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="text-sm">
                      {row.monthly_bucket ? row.monthly_bucket.slice(0, 7) : "-"}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
