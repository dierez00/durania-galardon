...
export default function ProducerMovilizacionPage() {
  ...
  // Initialise form UPP from context
  useEffect(() => {
  ...
  const createMovement = async () => {
    ...
    const response = await fetch("/api/producer/movements", {
      ...
      body: JSON.stringify({ uppId: formUppId, movementDate: movementDate || undefined, routeNote: routeNote || undefined }),
    });
  ...
  const uppName = (uppId: string | null) => {
    if (!uppId) return "-";
    return upps.find((u) => u.id === uppId)?.name ?? uppId.slice(0, 8);
  };
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      {row.qr_code ? (
                        <span className="font-mono text-xs bg-muted px-2 py-0.5 rounded">{row.qr_code}</span>
                      ) : (
